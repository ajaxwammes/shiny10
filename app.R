...
value=plyr::mapvalues(
    value, c("-1","0","1","3","5"),c("Selected country","Restricted to enter","Mandatory quarantine after arrival",
                                     "Mandatory certificate being COVID-19 free",
...
ui <- fluidPage(
  ...
  tags$head(includeHTML("google-analytics.html")),
    tags$head(HTML('<script data-ad-client="ca-pub-8812626540932611" async src="https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js"></script>')),
    HTML('<title> Travel advice during COVID-19 </title>'),
    theme = shinytheme("spacelab"),
        tabsetPanel(type = "tabs",
                                             ...
                                             sidebarLayout(
                                                 sidebarPanel(
                                                     selectInput(inputId = "location",label = "Select current location",
                                                                 choices = unique(data3$Traveling.from), multiple = FALSE, selected = "Netherlands"),
                                                     br(),
                                                     br(),
                                                     br(),
                                                     br(),
                                                 ...
                                                 mainPanel(tags$style(type = "text/css", "#mymap {height: calc(100vh - 145px) !important;}", 
                                                                      "#mymap {width: calc(100vh + 525px) !important;}"), 
                    ...
                    tabPanel("General Visa Regulations (World Map)",
                             ...
                             fluidRow(column(6,
                                             sidebarLayout(
                                                 ...
                                                 mainPanel(tags$style(type = "text/css", "#mymap2 {height: calc(100vh - 145px) !important;}", 
                                                                      "#mymap2 {width: calc(100vh + 525px) !important;}"), 
                ...
                into global travel restrictions, from the most reliable sources.",
                 br(),
                 br(),
                "CovidTravel is daily updated by data scraping from 200+ official government and embassy websites 
                around the world. Clicking on a country will show the linked source", 
                 br(),
                "When governmental websites do not provide information in English, embassy websites (mostly US and UK) are used as they 
                 are most up-to-date and provide good insights for all international travellers",
                br(),
                ...
                br(),
                br(),
                "CovidTravel and its intellectual property shall not be used without permission from CovidTravel by anyone.",
                br(),
                br(),
                "CovidTravel is open for commercial collaborations. For this, please find our contact information in the:", tags$div(div(id="Foo4", tags$a("Contact Tab"))),),
                    
                    tabPanel("Contact",
                             titlePanel(title=img(src="covidtravel.png")),
                             HTML("<script>$('#Foo4').click(function() {
                                                tabs = $('.tabbable .nav.nav-tabs li')
                                                 tabs.each(function() {
                                                    $(this).removeClass('active')
                                                 })
                                                ...
                                                $('#Foo4').trigger('change').trigger('shown');
...
label2 <- paste("More info about ", map2$Traveling.to,": ","<a href=",wiki_label,"target=\"_blank\">",wiki_label, "</a>")
  ...
  addProviderTiles("CartoDB.Voyager") %>%
    ...
    addPolygons(data=map2, fillColor = ~pal2(map2$value),
                ...
                highlight = highlightOptions(
                    ...
                    fillOpacity = .8,
                    bringToFront = TRUE), 
label = ~mytext2, popup = ~label2)  %>% addLegend(position = "bottomleft", pal = pal2, values = map2$value,opacity = 0.5, 
