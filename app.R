    ...
    theme = shinytheme("spacelab"),
        tabsetPanel(type = "tabs",
                    ...
                    tabPanel("Essential Movement",
                             ...
                             br(),
                             "> Availability of transportation (flights, trains, boats, etc)",
                             br(),
                             "> Essential movements such as diplomacy, humanitarian help, returning home. In some cases transits, business trips and 
                             education also count as essential travel. Always check the source information by clicking on the country before you travel",
                             br(),
                             "> In some cases only certain forms of transportation are allowed to enter a country 
                             (in certain cases a country cannot be entered by crossing land, but only by air)",
                             br(),
                             ...
                             "> In some cases, health checks such as temperature checks are (in some cases randomly) executed at borders. 
                                When failing such a test, Many countries are currently screening for high temperatures at border checkpoint. When
                                failing such a test, CovidTravel's travel restriction do not apply and it is 
                                possible to be rejected entrance or manditory quarantine, regardless of the travel status.",
                             br(),  
                ...
                all around the world. The source of data for a specific country can be found by clicking on the country.
  ...
  setView(lng=pfg5$Longitude[1], lat=pfg5$Latitude[1], zoom=2.5) %>%
    ...
    addPolygons(data=map, fillColor = ~pal(map$value),
                ...
                highlight = highlightOptions(
                    ...
                    dashArray = "3",
                    fillOpacity = .8,
                    bringToFront = TRUE), popup = paste("More info about ", map$Traveling.to,": ", "<a href=", map$Source,">", map$Source, "</a>"),
                label = ~mytext)  %>%
